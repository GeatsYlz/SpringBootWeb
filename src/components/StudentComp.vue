...
export default {
  ...
  methods: {
    ...
    getStudentList() {
      axios
        .get("http://localhost:9090/student")
        .then((res) => {
    ...
    handleRowDelete(row) {
      console.log(row);
      axios
        .delete("http://localhost:9090/deletestudent", { data: { id: row.id } })
        .then((res) => {
          console.log(res.data);
          // 如果删除成功，从数组中删除该学生
    ...
    handleRowUpdate(row) {
      console.log(row);
      axios
        .put("http://localhost:9090/updatestudent", row)
        .then((res) => {
          console.log(res.data);
          row.isEdit = false;
        })
    ...
    handleRowInsert(row) {
      console.log(row);
      axios
        .post("http://localhost:9090/addstudent", row)
        .then((res) => {
          console.log(res.data);
          row.isEdit = false;
          row.isNew = false;
